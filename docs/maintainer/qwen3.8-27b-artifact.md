...
architecture but carry a custom chat template, supply the standard template explicitly:

```bash
.venv/bin/python -m tools.convert.qwen3_8_27b.convert_gguf \
  --model /path/to/third-party-Q4_K_M.gguf \
  --mmproj /path/to/mmproj-Qwen3.8-27B-BF16.gguf \
  --out /path/to/output.ninfer \
  --chat-template /path/to/chat_template_qwen38.jinja
```

The companion mmproj GGUF is required even when Vision is not used at runtime. The ranking file
defaults to `tools/freq_corpus/fixtures/ranking/ranking.train.counts.i64`.

### GGUF source constraints

The converter only accepts GGUF files that satisfy all of the following:

- **Architecture**: `qwen35` with all expected metadata fields matching Qwen3.8-27B (block count,
  embedding length, head dimensions, SSM parameters, full-attention interval, rope sections).
- **Quantization types**: exclusively GGML types 12 (`Q4_K`), 14 (`Q6_K`), and 0 (`F32`), with
  exactly 439 Q4_K, 67 Q6_K, and 360 F32 tensors (866 total). Mixed-precision GGUFs using IQ2/IQ3/IQ4
  or other quantization types are rejected. The `preflight()` check enforces these counts exactly.
- **Chat template**: the GGUF's `tokenizer.chat_template` must hash to one of the two registered
  digests. Most third-party GGUFs carry a custom template and require `--chat-template`.

If a GGUF fails conversion, verify its `general.file_type` is 15 (Q4_K_M), its tensor types
are only Q4_K/Q6_K/F32, and its chat template matches a registered digest. For GGUFs with
different quantization (e.g. IQ3_S, Q5_K_M), dequantize to Q4_K_M first using llama.cpp before
converting.
...
preserves the embedded template, whose digest must match the registered Qwen3.8 reasoning-effort
template (`c3cf9e34abf4f9e36c2d72165aa9c132d3e2a725b6c2586aaa3a8af9d7a81041`) or the
thinking-toggle template (`e84f32a23fdda27689f868aa4a1a5621f41133e51a48d7f3efcbfea2839574259`).
The engine rejects any other template digest. Third-party GGUFs that carry a custom chat template
must supply `--chat-template /path/to/standard_qwen38.jinja` to override the GGUF's embedded
template with the registered one. The override replaces both the `frontend/chat_template.jinja`
resource and the `tokenizer_config.json.chat_template` field so they remain consistent. Image
geometry and normalization come from the companion GGUF; inactive image-size/video-sampling values
use the existing family defaults. These resources do not enable Vision execution for this identity.
